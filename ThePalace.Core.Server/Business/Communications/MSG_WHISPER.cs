...
namespace ThePalace.Core.Server.Business
{
    ...
    public class MSG_WHISPER : IBusinessReceive
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            ...
            var inboundPacket = (Protocols.MSG_WHISPER)args.FirstOrDefault();
            var chatStr = inboundPacket.text;

            Logger.ConsoleLog($"MSG_WHISPER[{_sessionState.UserID} -> {inboundPacket.targetID}]: {chatStr}");
            ...
            {
                _sessionState.Send(inboundPacket, EventTypes.MSG_WHISPER, (Int32)_sessionState.UserID);

                targetSessionState.Send(inboundPacket, EventTypes.MSG_WHISPER, (Int32)_sessionState.UserID);
            }
        }
    }
}
