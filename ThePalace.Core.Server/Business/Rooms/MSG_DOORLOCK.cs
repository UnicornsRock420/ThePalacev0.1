...
namespace ThePalace.Core.Server.Business
{
    ...
    public class MSG_DOORLOCK : IBusinessReceive
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            ...
            var inboundPacket = (Protocols.MSG_DOORLOCK)args.FirstOrDefault();
            ...
            if (inboundPacket.roomID == _sessionState.RoomID)
            {
                ...
                if (!room.NotFound)
                {
                    ...
                    if (hotspotType == (short)HotspotTypes.HS_Bolt)
                    {
                        ...
                        Network.SessionManager.SendToRoomID(_sessionState.RoomID, 0, inboundPacket, EventTypes.MSG_DOORLOCK, (Int32)_sessionState.UserID);
                    }
                    else
                    {
