...
{
    [Description("opSs")]
    public sealed class MSG_SPOTSETDESC : IBusinessSend
    {
        public void Send(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_SPOTSETDESC[" + nameof(sessionState) + "]");

            var outboundPacket = args.FirstOrDefault() as Header;
            if (outboundPacket != null)
            {
#if DEBUG
                Debug.WriteLine($"MSG_SPOTSETDESC[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
