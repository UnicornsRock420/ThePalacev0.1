...
{
    [Description("nPct")]
    public sealed class MSG_PICTNEW : IBusinessReceive, IBusinessSend
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_PICTNEW[" + nameof(sessionState) + "]");

            var inboundHeader = args.FirstOrDefault() as Header;
            if (inboundHeader == null) return;

            //var inboundPacket = inboundHeader.protocolReceive as Protocols.MSG_PICTNEW;
            //if (inboundPacket == null) return;

            ScriptEvents.Current.Invoke(IptEventTypes.MsgPictNew, sessionState, inboundHeader, sessionState.ScriptState);

#if DEBUG
            Debug.WriteLine($"MSG_PICTNEW[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
        }
