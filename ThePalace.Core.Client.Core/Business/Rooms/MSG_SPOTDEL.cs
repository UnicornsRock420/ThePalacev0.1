...
{
    [Description("opSd")]
    public sealed class MSG_SPOTDEL : IBusinessReceive, IBusinessSend
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_SPOTDEL[" + nameof(sessionState) + "]");

            var inboundHeader = args.FirstOrDefault() as Header;
            if (inboundHeader == null) return;

            //var inboundPacket = inboundHeader.protocolReceive as Protocols.MSG_SPOTDEL;
            //if (inboundPacket == null) return;

            ScriptEvents.Current.Invoke(IptEventTypes.MsgSpotDel, sessionState, inboundHeader, sessionState.ScriptState);

#if DEBUG
            Debug.WriteLine($"MSG_SPOTDEL[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
        }
