...
{
    [Description("sInf")]
    public sealed class MSG_EXTENDEDINFO : IBusinessReceive, IBusinessSend
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_EXTENDEDINFO[" + nameof(sessionState) + "]");

            var inboundHeader = args.FirstOrDefault() as Header;
            if (inboundHeader == null) return;

            //var inboundPacket = inboundHeader.protocolReceive as Protocols.MSG_EXTENDEDINFO;
            //if (inboundPacket == null) return;

            //TODO:

#if DEBUG
            Debug.WriteLine($"MSG_EXTENDEDINFO[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
        }
