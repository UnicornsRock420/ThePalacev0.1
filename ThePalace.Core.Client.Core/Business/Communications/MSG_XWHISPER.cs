...
{
    [Description("xwis")]
    public sealed class MSG_XWHISPER : IBusinessReceive, IBusinessSend
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_XWHISPER[" + nameof(sessionState) + "]");

            var inboundHeader = args.FirstOrDefault() as Header;
            if (inboundHeader == null) return;

            var inboundPacket = inboundHeader.protocolReceive as Protocols.Communications.MSG_XWHISPER;
...
#if DEBUG
            Debug.WriteLine($"MSG_XWHISPER[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
        }
