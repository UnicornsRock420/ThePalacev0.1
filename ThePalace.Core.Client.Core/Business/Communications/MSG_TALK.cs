...
{
    [Description("talk")]
    public sealed class MSG_TALK : IBusinessReceive, IBusinessSend
    {
        public void Receive(ISessionState sessionState, params object[] args)
        {
            var _sessionState = sessionState as IClientSessionState;
            if (_sessionState == null)
                throw new ArgumentNullException("MSG_TALK[" + nameof(sessionState) + "]");

            var inboundHeader = args.FirstOrDefault() as Header;
            if (inboundHeader == null) return;

            var inboundPacket = inboundHeader.protocolReceive as Protocols.Communications.MSG_TALK;
...
#if DEBUG
            Debug.WriteLine($"MSG_TALK[{_sessionState.SessionID}:{_sessionState.UserID}]");
#endif
        }
